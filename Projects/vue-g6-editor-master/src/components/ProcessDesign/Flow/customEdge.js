  ...
  init() {
    ...
    G6.registerEdge('customEdge', {
      draw(cfg, group) {
        ...
        const keyShape = group.addShape('path', {
          attrs: {
            ...
            path: path,
            stroke: cfg.color,
            unSelectStroke: cfg.unSelectColor,
            selectStroke: cfg.selectColor,
            lineAppendWidth: 10,
            label: cfg.label,
            endArrow: {
              path: endArrowPath,
        ...
        if (cfg.label) {
          ...
          let textY = null;
          if (end.x > start.x) {
            textX = start.x + ((end.x-start.x)/2);
          } else {
            textX = end.x + ((start.x-end.x)/2);
          }
          if (end.y > start.y) {
            textY = start.y + ((end.y-start.y)/2);
          } else {
            textY = end.y + ((start.y-end.y)/2);
          }
          group.addShape("text", {
           attrs: {
             id: 'label' + uniqueId(),
      ...
      setState(name, value, item) {
        ...
        const shape = group.get("children")[0];
        const selectStyles = () => {
          shape.attr("stroke", shape._attrs.selectStroke);
        };
        const unSelectStyles = () => {
          shape.attr("stroke", shape._attrs.unSelectStroke);
        };
