...
//            	var actions = [];
//                	if ('1' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "首次备案报告" + "</span>");
//                }
//                	else if ('2' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "持续报告" + "</span>");
//                }
//                	else if ('3' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "定期报告" + "</span>");
//                }
//                	else if ('4' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "重大事项报告" + "</span>");
//                }
//                	else if ('5' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "其它事项报告" + "</span>");
//                }
//                	else if ('6' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "非公开发行债" + "</span>");
//                }
//                	else if ('7' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "收益凭证" + "</span>");
//                }
//                	else if ('8' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "证券公司外部接入系统" + "</span>");
//                }
//                	else if ('9' == value) {
//                    	actions.push("<span class='badge badge-primary'>" + "区域性市场业务" + "</span>");
//                }
//                return actions.join('');
//            },
            selecttemplate: function(value) {
            	var actions = [];
                	if ('0' == value) {
                    	...
                    	actions.push("<span class='badge badge-primary'>" + "禁用" + "</span>");
                }
    	...
    	form: {
    		...
    		reset: function(formId) {
            	...
            	if ($("#" + currentId + " .select-time").length > 0 && startDate_q && endDate_q) {
            		startDate_q.config.max.year = endDate_q.config.max.year;
            		startDate_q.config.max.month = endDate_q.config.max.month - 1;
            		startDate_q.config.max.date = endDate_q.config.max.date;
            		
	                endDate_q.config.min.year = 1900;
	                endDate_q.config.min.month = 0;
	                endDate_q.config.min.date = 1;
            	}
        ...
        operate: {
            ...
            //查看明细
            detailed: function(id,monthly, width, height) {
        		debugger;
        		var monthlys = "{monthly}";
                var _url = $.common.isEmpty(id) ? $.table._option.detaileds : $.table._option.detaileds.replace("{id}",id);
                var _urls = $.common.isEmpty(monthly) ? $.table._option.detaileds : monthlys.replace("{monthly}",monthly);
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    $.get(options.url, function(data) {
        			...
        			// 展开第四级节点
        			nodes = tree.getNodesByParam("level", 3);
        			for (var i = 0; i < nodes.length; i++) {
        				if(_expandLevel > 2) {
        					tree.expandNode(nodes[i], true, false, false);
        				}
        	...
        	notAllowLastLevel: function(_tree) { 
    		    var nodes = _tree.getSelectedNodes();
    		    for (var i = 0; i < nodes.length; i++) {
    		    	if (nodes[i].level >= _tree._level_1+1) {
    		    		$.modal.msgError("不能选择当前层级节点（" + nodes[i].name + "）");
    		            return false;
    		        }
    		    }
        		return true;
