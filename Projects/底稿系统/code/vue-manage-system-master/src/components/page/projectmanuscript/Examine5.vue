    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator=">>">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-redpacket_fill"></i> {{projectInfo.projectname}}--内核结束
                </el-breadcrumb-item>
                <el-breadcrumb-item>{{projectInfo.stagename}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <v-examine5-tree :projectid="$route.query.id" :projectStage="$route.query.projectStage" :roles="roles"></v-examine5-tree>
...
<script>
import vExamine5Tree from '@/components/common/Examine5Tree.vue';
export default {
  name: "exmine5",
  components: {
    vExamine5Tree
  ...
  data() {
    return {
      roles: {
          pageid_roles_5: false,//页面对应流程节点值显示权限
  ...
  methods: {
    ...
    //     bus.$emit('examine5Tree_load_'+tab.index);
    // },
  }
};
</script>
