    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator=">>">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-redpacket_fill"></i> {{projectInfo.projectname}}--内核审核
                </el-breadcrumb-item>
                <el-breadcrumb-item>{{projectInfo.stagename}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <v-examine4-tree :projectid="$route.query.id" :projectStage="$route.query.projectStage" :roles="roles"></v-examine4-tree>
...
<script>
import vExamine4Tree from '@/components/common/Examine4Tree.vue';
export default {
  name: "exmine4",
  components: {
    vExamine4Tree
  ...
  data() {
    return {
      roles: {
          pageid_roles_4: false,//页面对应流程节点值显示权限
  ...
  methods: {
    ...
    //     bus.$emit('examine4Tree_load_'+tab.index);
    // },
  }
};
</script>
