    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator=">>">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-redpacket_fill"></i> {{projectInfo.projectname}}--质控审核
                </el-breadcrumb-item>
                <el-breadcrumb-item>{{projectInfo.stagename}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <v-examine2-tree :projectid="$route.query.id" :projectStage="$route.query.projectStage" :roles="roles"></v-examine2-tree>
...
import vExamine2Tree from '@/components/common/Examine2Tree.vue';
export default {
  name: "exmine2",
  components: {
    vExamine2Tree
  ...
  data() {
    return {
      roles: {
          pageid_roles_2: false,//页面对应流程节点值显示权限
  ...
  methods: {
    ...
    //     bus.$emit('examine2Tree_load_'+tab.index);
    // },
  }
};
</script>
