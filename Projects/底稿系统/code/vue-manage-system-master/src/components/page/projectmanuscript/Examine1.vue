    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator=">>">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-redpacket_fill"></i> {{projectInfo.projectname}}--项目组审核
                </el-breadcrumb-item>
                <el-breadcrumb-item>{{projectInfo.stagename}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <v-examine1-tree :projectid="$route.query.id" :projectStage="$route.query.projectStage" :roles="roles"></v-examine1-tree>
...
import vExamine1Tree from '@/components/common/Examine1Tree.vue';
export default {
  name: "exmine1",
  components: {
    vExamine1Tree
  ...
  data() {
    return {
      roles: {
          pageid_roles_1: false,//页面对应流程节点值显示权限
  ...
  methods: {
    ...
    //     bus.$emit('examine1Tree_load_'+tab.index);
    // },
  }
};
</script>
