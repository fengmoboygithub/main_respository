            ...
            <el-col :span="12" ref="rootDirRef">
              ...
              <div v-if="showFile">
                <el-row :gutter="20">
                    ...
                    <el-col :span="4">
                      <el-button v-if="fileInfo.status == '11' && status_show" type="primary" @click="onManuscriptStatus">标记为已阅</el-button>
                      <el-button v-if="fileInfo.status == '12' && status_show" type="primary" @click="onManuscriptStatus">标记为未阅</el-button>
                    </el-col>
                    <el-col :span="2">
                ...
                <el-row>
                  <v-pdf v-if="fileInfo.fileType == '11'" ref="pdfMe" :url="fileInfo.fileUrl" pdfStyle="display: inline-block; width: 100%; height: 100%;border: solid 1px;"></v-pdf>
                  <v-audio v-if="fileInfo.fileType == '12'" ref="audioMe" :url="fileInfo.fileUrl" audioStyle="display: inline-block; width: 100%; height: 100%;border: solid 1px;"></v-audio>
                  <v-video v-if="fileInfo.fileType == '13'" ref="videoMe" :url="fileInfo.fileUrl" videoStyle="display: inline-block; width: 100%; height: 100%;border: solid 1px;"></v-video>
                </el-row>
                ...
                <el-row >
                    <el-button type="primary" v-if="status_show" @click="onRemarks">保存</el-button>
                </el-row>
              </div>
              <!-- 展示目录上传文件 -->
              <div v-if="showDir">
                  ...
                  <div class="content-title" v-if="status_show">文件上传</div>
                  <div class="form-box" v-if="status_show">
                  ...
                  <el-table
                    ...
                    <el-row>
                      <el-button type="primary" v-if="status_show" @click="onSubmit">提交</el-button>
                    </el-row>
...
export default {
  ...
  mounted() {
    ...
    this.initTree();
    // bus.$on('uploadTree_load_'+this.index, () => {
    //     this.initTree();
    // });
  },
  // activated() {
  ...
  methods: {
    ...
    selectNodeById(selectId) {
      if(selectId){
        ...
        if(node) {
          ...
          this.onClick({'type':'me'}, node.id, node);
          //展开该节点
          this.ztreeObj && this.ztreeObj.expandAll(false);
          this.ztreeObj && this.ztreeObj.expandNode(node, true);
        }
      }
    },
    //获取目录树数据
    ...
    optMenthod(operId, msg){
      //调用后台接口流程相关操作 operId==>> 1:提交 2:退回 3:保存
      var nodes = this.ztreeObj.getCheckedNodes(true);
      if(nodes == null || nodes.length == 0){
        this.$message.error("请勾选要操作的目录");
      } else {
        ...
        this.net.updateProcess_ProcessNodeInfo(param).then(res => {
            ...
            if (res.code === 200) {
              ...
              this.initTree();
              this.selectNodeById(this.dirInfo.id);
            } else {
              this.$message.error(res.message);
            }
        // eslint-disable-next-line no-unused-vars
