          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12" ref="rootDirRef">
              ...
              <div v-if="showExamineDir">
                  ...
                  <div v-if="roles.pageid_roles_2 && roles.status_show"  class="content-title">保存意见</div>
                  <el-input
                    type="textarea"
                    v-if="roles.pageid_roles_2 && roles.status_show" 
                    :rows="10"
                    placeholder="请输入意见"
                    v-model="opinion">
                  </el-input>
                  <el-row v-if="roles.pageid_roles_2 && roles.status_show" >
                  ...
                  <el-table
                    ...
                    <el-row>
                      <el-button v-if="roles.pageid_roles_2 && roles.status_show" type="danger" plain @click="onBack">退回</el-button>
                      <!-- <el-button v-if="roles.pageid_roles_2 && roles.status_show" type="success" plain  @click="onAgree">同意</el-button> -->
                    </el-row>
              </div>
...
export default {
  ...
  mounted() {
    ...
    this.initTree();
    // bus.$on('examine2Tree_load_'+this.index, () => {
    //     this.initTree();
    // });
  },
  // activated() {
  ...
  methods: {
    ...
    onClick: function(evt, treeId, treeNode) {
      ...
      if(treeNode) {
        if(treeNode.type == 'file'){
          ...
          if(processnodeStr){
            this.net.getList_PageProcessnodeMap({pagevalue:'3'}).then(res => {
              if (res.code === 200) {
                var results = _.some(res.list, item => {
                  return item.processnode == processnodeStr;
                });
                if(results){
                  this.roles.pageid_roles_2 = true;
                } else {
                  this.roles.pageid_roles_2 = false;
                }
    ...
    //统一提交
    optMenthodAll(operId, msg){
      //调用后台接口流程相关操作 operId==>> 1:提交
      //查找所有目录集合不包括文件及根目录
    ...
    onSaveAll(){
      //保存
      //判断当前用户是否能进行保存
      this.optMenthodAll('3', '保存');
    },
    onAgree(){
      //同意
      //判断当前用户是否能进行同意
      this.optMenthodAll('1', '同意');
    },
    filter(node) {
