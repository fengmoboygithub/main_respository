          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12" ref="rootDirRef">
              ...
              <div v-if="showRootDir">
                  <div v-if="roles.pageid_roles_4 && roles.status_show"  class="content-title">保存意见</div>
                  <el-input
                    type="textarea"
                    v-if="roles.pageid_roles_4 && roles.status_show" 
                    :rows="10"
                    placeholder="请输入意见"
                    v-model="opinion">
                  </el-input>
                  <el-row>
                    <el-button v-if="roles.pageid_roles_4 && roles.status_show" type="danger" plain @click="onBack">不同意</el-button>
                    <el-button v-if="roles.pageid_roles_4 && roles.status_show" type="success" plain  @click="onAgree">同意</el-button>
                  </el-row>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  mounted() {
    ...
    this.initTree();
    // bus.$on('examine4Tree_load_'+this.index, () => {
    //     this.initTree();
    // });
  },
  // activated() {
  ...
  methods: {
    ...
    onClick: function(evt, treeId, treeNode) {
      ...
      if(treeNode) {
        if(treeNode.type == 'file'){
          ...
          if(processnodeStr){
            this.net.getList_PageProcessnodeMap({pagevalue:'5'}).then(res => {
              if (res.code === 200) {
                var results = _.some(res.list, item => {
                  return item.processnode == processnodeStr;
                });
                if(results){
                  this.roles.pageid_roles_4 = true;
                } else {
                  this.roles.pageid_roles_4 = false;
                }
    ...
    optMenthod(operId, msg){
      ...
      this.net.add_InternalAuditReviewResult(param).then(res => {
        loadingInstance.close();
        if (res.code === 200) {
