          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12" ref="rootDirRef">
              ...
              <div v-if="showDir">
                  ...
                  <div class="content-title">内核历史意见</div>
                  <el-table
                    ...
                    >
                        ...
                        <el-table-column prop="isagree" label="是否同意" :formatter="isagreeFormatter"></el-table-column>
                    </el-table>
              ...
              <div v-if="showRootDir">
                  ...
                  <div v-if="roles.pageid_roles_5 && roles.status_show"  class="content-title">保存意见</div>
                  <el-input
                    type="textarea"
                    v-if="roles.pageid_roles_5 && roles.status_show" 
                    :rows="10"
                    placeholder="请输入意见"
                    v-model="opinion">
                  </el-input>
                  <el-row v-if="roles.pageid_roles_5 && roles.status_show" >
                  ...
                  <el-table
                    ...
                    <el-row>
                      <el-button v-if="roles.pageid_roles_5 && roles.status_show" type="danger" plain @click="onBack">退回</el-button>
                      <el-button v-if="roles.pageid_roles_5 && roles.status_show" type="success" plain  @click="onAgree">结束</el-button>
                    </el-row>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  mounted() {
    ...
    this.initTree();
    // bus.$on('examine5Tree_load_'+this.index, () => {
    //     this.initTree();
    // });
  },
  // activated() {
  ...
  methods: {
    ...
    // 禁用状态格式化
    // eslint-disable-next-line no-unused-vars
    isagreeFormatter(row, column) {
      let isagree = row.isagree;
      if(isagree == '1'){
        return '同意';
      } else {
        return '不同意';
    ...
    getHisOpinionTableData() {
      ...
      //调用新的接口
      this.net.getListByPage_InternalAuditReviewResult(this.query).then(res => {
    ...
    onClick: function(evt, treeId, treeNode) {
      ...
      if(treeNode) {
        if(treeNode.type == 'file'){
          ...
          if(processnodeStr){
            this.net.getList_PageProcessnodeMap({pagevalue:'6'}).then(res => {
              if (res.code === 200) {
                var results = _.some(res.list, item => {
                  return item.processnode == processnodeStr;
                });
                if(results){
                  this.roles.pageid_roles_5 = true;
                } else {
                  this.roles.pageid_roles_5 = false;
                }
    ...
    optMenthod(operId, msg){
      //调用后台接口流程相关操作 operId==>> 1:结束 2:退回 3:保存
      //查找所有目录集合不包括文件及根目录
