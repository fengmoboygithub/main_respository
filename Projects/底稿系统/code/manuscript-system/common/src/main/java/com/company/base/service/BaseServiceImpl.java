...
public class BaseServiceImpl<T extends BaseModel,E extends BaseExample> implements IBaseService<T,E>{
	...
	@Override
	public void updateByExampleSelective(T record, E example) throws BaseException {

		int success = getMapper().updateByExampleSelective(record, example);
		if (success <= 0) {
			logger.error("更新实体错误{}",  record.toString());
			throw new BaseException("更新实体错误");
		}
		return;
	...
	@Override
	public PageInfo<T> getListByPageWithBlobs(int currentPage, int itemsPerPage, E example) throws BaseException {
