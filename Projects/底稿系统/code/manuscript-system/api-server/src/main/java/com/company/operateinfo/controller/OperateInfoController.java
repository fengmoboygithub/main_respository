package com.company.operateinfo.controller;
...
import com.company.operateinfo.model.OperateInfoExample.Criteria;
 ...
 *
 */
@Api(value="操作信息数据维护",tags={"操作信息数据操作接口"})
@RestController("OperateInfoController")
@RequestMapping("/api/OperateInfo")
public class OperateInfoController {

    public static Logger logger = LoggerFactory.getLogger(OperateInfoController.class);

    @Autowired
    private OperateInfoService<OperateInfo, OperateInfoExample> service;
	
	/**
	 * 	操作信息查询详情
	 */
	@ApiOperation("操作信息信息详情")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseDataModel<OperateInfo> get(
    	@RequestParam(required = true) @ApiParam(name="id",value="主键",required=true) String id
    	) {
        ResponseDataModel<OperateInfo> responseModel = new ResponseDataModel<>();
        try {
            OperateInfo entity = service.get(id);
	 ...
	 * 	操作信息分页查询
	 */
	@ApiOperation("分页查询操作信息数据")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public ResponseDataPageListForBootModel<OperateInfo> getListByPage(
        ...
        ResponseDataPageListForBootModel<OperateInfo> responseModel = new ResponseDataPageListForBootModel<OperateInfo>();
        
		try {
            OperateInfoExample example = new OperateInfoExample();
            //按主键倒序 查询
        	...
        	PageInfo<OperateInfo> page = service.getListByPage(currentPage, itemsPerPage, example);
        	responseModel.setSuccess(true);
	 ...
	 * 	操作信息添加数据
	 */
	@ApiOperation("添加操作信息数据")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseDataModel<OperateInfo> add(@RequestBody @ApiParam(name="操作信息对象",value="传入json格式",required=true) OperateInfo model,HttpServletRequest request) {
        ResponseDataModel<OperateInfo> responseModel = new ResponseDataModel<OperateInfo>();
        Long currUserId = (Long)request.getSession().getAttribute(BaseConstant.LOGIN_KEY);
        try {
            OperateInfo record = new OperateInfo();
...
//			record.setCreateUser(currUserId);
	 ...
	 * 	操作信息修改数据
	 */
	@ApiOperation("修改操作信息数据")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseDataModel<OperateInfo> update(@RequestBody @ApiParam(name="操作信息对象",value="传入json格式",required=true) OperateInfo model,HttpServletRequest request) {
        ResponseDataModel<OperateInfo> responseModel = new ResponseDataModel<OperateInfo>();
        Long currUserId = (Long)request.getSession().getAttribute(BaseConstant.LOGIN_KEY);
        try {
            OperateInfo record = new OperateInfo();
