...
(function($){
	...
	window.wxc.xcConfirm = function(popHtml, type, options) {
		...
		if(type == "custom"){
			$popBox.css({"height":"400px"});
			$txtBox = $("<div>").addClass("txtBoxImg");
		} else {
			$popBox.css({"height":"300px"});
			$txtBox = $("<div>").addClass("txtBox");
		}
		
		
		
		var $ok = $("<a>").addClass("sgBtn").addClass("ok").text(config.okTxt);//确定按钮
		var $cancel = $("<a>").addClass("sgBtn").addClass("cancel").text(config.cancelTxt);//取消按钮
		var $input = $("<input>").addClass("inputBox");//输入框
		var $clsBtn = null;
		if(config.isShowCloseBtn){
