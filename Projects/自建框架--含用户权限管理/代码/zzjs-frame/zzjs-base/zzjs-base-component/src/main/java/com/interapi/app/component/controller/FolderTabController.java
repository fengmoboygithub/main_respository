...
import com.interapi.app.component.model.FolderTabExample.Criteria;
...
@RestController("FolderTabController")
@RequestMapping("/api/FolderTab")
@ResourceMapping(code = "FolderTab", name = "目录_模块", tableName = "folder_tab", menuName = "站点管理",
roleName = "目录_页面", roleFlag = "ROLE_FolderTab_PAGE", homePage = "/pages/FolderTab",
pageSort = "3", menuSort = "1")
public class FolderTabController {

    public static Logger logger = LoggerFactory.getLogger(FolderTabController.class);

    @Autowired
    private FolderTabService<FolderTab, FolderTabExample> service;
	
	@Secured("ROLE_FolderTab_QUERY_DETAIL")
	@Cacheable(value = "FolderTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FolderTab/get", roleName = "目录_查询_详情", 
					roleFlag = "ROLE_FolderTab_QUERY_DETAIL")
    public ResponseDataModel<FolderTab> get(Long id) {
        ResponseDataModel<FolderTab> responseModel = new ResponseDataModel<>();
        try {
            FolderTab entity=service.get(id);
	...
	@Secured("ROLE_FolderTab_QUERY_PAGE")
	//@Cacheable(value = "FolderTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/FolderTab/getListByPage", roleName = "目录_查询_分页", 
					roleFlag = "ROLE_FolderTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<FolderTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<FolderTab> responseModel = new ResponseDataPageListForBootModel<FolderTab>();
        ...
        FolderTabExample example = new FolderTabExample();
        ...
        PageInfo<FolderTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_FolderTab_QUERY_LIST")
    //@Cacheable(value = "FolderTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/FolderTab/getList", roleName = "目录_查询_列表", 
					roleFlag = "ROLE_FolderTab_QUERY_LIST")
    public ResponseDataListModel<FolderTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<FolderTab> responseModel = new ResponseDataListModel<FolderTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<FolderTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_FolderTab_ADD")
	@CachePut(value = "FolderTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FolderTab/add", roleName = "目录_添加", 
				roleFlag = "ROLE_FolderTab_ADD")
    public ResponseDataModel<FolderTab> add(@RequestBody FolderTab model) {
        ResponseDataModel<FolderTab> responseModel = new ResponseDataModel<FolderTab>();
        try {
            FolderTab record = new FolderTab();
	...
	@Secured("ROLE_FolderTab_EDIT")
	@CachePut(value = "FolderTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FolderTab/update", roleName = "目录_编辑", 
			roleFlag = "ROLE_FolderTab_EDIT")
    public ResponseDataModel<FolderTab> update(@RequestBody FolderTab model) {
        ResponseDataModel<FolderTab> responseModel = new ResponseDataModel<FolderTab>();
        try {
            FolderTab record = new FolderTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_FolderTab_DELETE")
	@CacheEvict(value = "FolderTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FolderTab/del", roleName = "目录_删除", 
				roleFlag = "ROLE_FolderTab_DELETE")
