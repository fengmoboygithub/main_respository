...
import com.interapi.app.component.model.MediaTabExample.Criteria;
...
@RestController("MediaTabController")
@RequestMapping("/api/MediaTab")
@ResourceMapping(code = "MediaTab", name = "资源_模块", tableName = "media_tab", menuName = "站点管理",
roleName = "资源_页面", roleFlag = "ROLE_MediaTab_PAGE", homePage = "/pages/MediaTab",
pageSort = "4", menuSort = "1")
public class MediaTabController {

    public static Logger logger = LoggerFactory.getLogger(MediaTabController.class);

    @Autowired
    private MediaTabService<MediaTab, MediaTabExample> service;
	
	@Secured("ROLE_MediaTab_QUERY_DETAIL")
	@Cacheable(value = "MediaTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/MediaTab/get", roleName = "资源_查询_详情", 
					roleFlag = "ROLE_MediaTab_QUERY_DETAIL")
    public ResponseDataModel<MediaTab> get(Long id) {
        ResponseDataModel<MediaTab> responseModel = new ResponseDataModel<>();
        try {
            MediaTab entity=service.get(id);
	...
	@Secured("ROLE_MediaTab_QUERY_PAGE")
	//@Cacheable(value = "MediaTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/MediaTab/getListByPage", roleName = "资源_查询_分页", 
					roleFlag = "ROLE_MediaTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<MediaTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<MediaTab> responseModel = new ResponseDataPageListForBootModel<MediaTab>();
        ...
        MediaTabExample example = new MediaTabExample();
        ...
        PageInfo<MediaTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_MediaTab_QUERY_LIST")
    //@Cacheable(value = "MediaTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/MediaTab/getList", roleName = "资源_查询_列表", 
					roleFlag = "ROLE_MediaTab_QUERY_LIST")
    public ResponseDataListModel<MediaTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<MediaTab> responseModel = new ResponseDataListModel<MediaTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<MediaTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_MediaTab_ADD")
	@CachePut(value = "MediaTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/MediaTab/add", roleName = "资源_添加", 
				roleFlag = "ROLE_MediaTab_ADD")
    public ResponseDataModel<MediaTab> add(@RequestBody MediaTab model) {
        ResponseDataModel<MediaTab> responseModel = new ResponseDataModel<MediaTab>();
        try {
            MediaTab record = new MediaTab();
	...
	@Secured("ROLE_MediaTab_EDIT")
	@CachePut(value = "MediaTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/MediaTab/update", roleName = "资源_编辑", 
			roleFlag = "ROLE_MediaTab_EDIT")
    public ResponseDataModel<MediaTab> update(@RequestBody MediaTab model) {
        ResponseDataModel<MediaTab> responseModel = new ResponseDataModel<MediaTab>();
        try {
            MediaTab record = new MediaTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_MediaTab_DELETE")
	@CacheEvict(value = "MediaTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/MediaTab/del", roleName = "资源_删除", 
				roleFlag = "ROLE_MediaTab_DELETE")
