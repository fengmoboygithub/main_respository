...
import com.interapi.app.component.model.DictionaryTabExample.Criteria;
...
@ResourceMapping(code = "DictionaryTab", name = "数据字典_模块", tableName = "dictionary_tab", menuName = "配置管理",
roleName = "数据字典_页面", roleFlag = "ROLE_DictionaryTab_PAGE", homePage = "/pages/DictionaryTab",
pageSort = "1", menuSort = "5")
public class DictionaryTabController {
	...
	@Secured("ROLE_DictionaryTab_QUERY_DETAIL")
	@Cacheable(value = "DictionaryTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DictionaryTab/get", roleName = "数据字典_查询_详情", 
					roleFlag = "ROLE_DictionaryTab_QUERY_DETAIL")
    public ResponseDataModel<DictionaryTab> get(Long id) {
	...
	@Secured("ROLE_DictionaryTab_QUERY_PAGE")
	//@Cacheable(value = "DictionaryTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/DictionaryTab/getListByPage", roleName = "数据字典_查询_分页", 
					roleFlag = "ROLE_DictionaryTab_QUERY_PAGE")
    ...
    @Secured("ROLE_DictionaryTab_QUERY_LIST")
    //@Cacheable(value = "DictionaryTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/DictionaryTab/getList", roleName = "数据字典_查询_列表", 
					roleFlag = "ROLE_DictionaryTab_QUERY_LIST")
    public ResponseDataListModel<DictionaryTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<DictionaryTab> responseModel = new ResponseDataListModel<DictionaryTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<DictionaryTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_DictionaryTab_ADD")
	@CachePut(value = "DictionaryTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DictionaryTab/add", roleName = "数据字典_添加", 
				roleFlag = "ROLE_DictionaryTab_ADD")
    public ResponseDataModel<DictionaryTab> add(@RequestBody DictionaryTab model) {
	...
	@Secured("ROLE_DictionaryTab_EDIT")
	@CachePut(value = "DictionaryTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DictionaryTab/update", roleName = "数据字典_编辑", 
			roleFlag = "ROLE_DictionaryTab_EDIT")
    public ResponseDataModel<DictionaryTab> update(@RequestBody DictionaryTab model) {
        ResponseDataModel<DictionaryTab> responseModel = new ResponseDataModel<DictionaryTab>();
        try {
            DictionaryTab record = new DictionaryTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_DictionaryTab_DELETE")
	@CacheEvict(value = "DictionaryTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DictionaryTab/del", roleName = "数据字典_删除", 
				roleFlag = "ROLE_DictionaryTab_DELETE")
