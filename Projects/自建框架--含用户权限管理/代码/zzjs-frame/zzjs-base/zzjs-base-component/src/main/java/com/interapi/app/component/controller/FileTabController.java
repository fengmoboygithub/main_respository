...
import com.interapi.app.component.model.FileTabExample.Criteria;
...
@RestController("FileTabController")
@RequestMapping("/api/FileTab")
@ResourceMapping(code = "FileTab", name = "页面_模块", tableName = "file_tab", menuName = "站点管理",
roleName = "页面_页面", roleFlag = "ROLE_FileTab_PAGE", homePage = "/pages/FileTab",
pageSort = "2", menuSort = "1")
public class FileTabController {

    public static Logger logger = LoggerFactory.getLogger(FileTabController.class);

    @Autowired
    private FileTabService<FileTab, FileTabExample> service;
	
	@Secured("ROLE_FileTab_QUERY_DETAIL")
	@Cacheable(value = "FileTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FileTab/get", roleName = "页面_查询_详情", 
					roleFlag = "ROLE_FileTab_QUERY_DETAIL")
    public ResponseDataModel<FileTab> get(Long id) {
        ResponseDataModel<FileTab> responseModel = new ResponseDataModel<>();
        try {
            FileTab entity=service.get(id);
	...
	@Secured("ROLE_FileTab_QUERY_PAGE")
	//@Cacheable(value = "FileTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/FileTab/getListByPage", roleName = "页面_查询_分页", 
					roleFlag = "ROLE_FileTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<FileTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<FileTab> responseModel = new ResponseDataPageListForBootModel<FileTab>();
        ...
        FileTabExample example = new FileTabExample();
        ...
        PageInfo<FileTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_FileTab_QUERY_LIST")
    //@Cacheable(value = "FileTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/FileTab/getList", roleName = "页面_查询_列表", 
					roleFlag = "ROLE_FileTab_QUERY_LIST")
    public ResponseDataListModel<FileTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<FileTab> responseModel = new ResponseDataListModel<FileTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<FileTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_FileTab_ADD")
	@CachePut(value = "FileTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FileTab/add", roleName = "页面_添加", 
				roleFlag = "ROLE_FileTab_ADD")
    public ResponseDataModel<FileTab> add(@RequestBody FileTab model) {
        ResponseDataModel<FileTab> responseModel = new ResponseDataModel<FileTab>();
        try {
            FileTab record = new FileTab();
	...
	@Secured("ROLE_FileTab_EDIT")
	@CachePut(value = "FileTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FileTab/update", roleName = "页面_编辑", 
			roleFlag = "ROLE_FileTab_EDIT")
    public ResponseDataModel<FileTab> update(@RequestBody FileTab model) {
        ResponseDataModel<FileTab> responseModel = new ResponseDataModel<FileTab>();
        try {
            FileTab record = new FileTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_FileTab_DELETE")
	@CacheEvict(value = "FileTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/FileTab/del", roleName = "页面_删除", 
				roleFlag = "ROLE_FileTab_DELETE")
