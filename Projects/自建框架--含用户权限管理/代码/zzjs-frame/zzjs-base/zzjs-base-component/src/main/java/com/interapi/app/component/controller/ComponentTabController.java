...
import com.interapi.app.component.model.ComponentTabExample.Criteria;
...
@RestController("ComponentTabController")
@RequestMapping("/api/ComponentTab")
@ResourceMapping(code = "ComponentTab", name = "组件_模块", tableName = "component_tab", menuName = "组件管理",
roleName = "组件_页面", roleFlag = "ROLE_ComponentTab_PAGE", homePage = "/pages/ComponentTab",
pageSort = "1", menuSort = "2")
public class ComponentTabController {

    public static Logger logger = LoggerFactory.getLogger(ComponentTabController.class);

    @Autowired
    private ComponentTabService<ComponentTab, ComponentTabExample> service;
	
	@Secured("ROLE_ComponentTab_QUERY_DETAIL")
	@Cacheable(value = "ComponentTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ComponentTab/get", roleName = "组件_查询_详情", 
					roleFlag = "ROLE_ComponentTab_QUERY_DETAIL")
    public ResponseDataModel<ComponentTab> get(Long id) {
        ResponseDataModel<ComponentTab> responseModel = new ResponseDataModel<>();
        try {
            ComponentTab entity=service.get(id);
	...
	@Secured("ROLE_ComponentTab_QUERY_PAGE")
	//@Cacheable(value = "ComponentTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/ComponentTab/getListByPage", roleName = "组件_查询_分页", 
					roleFlag = "ROLE_ComponentTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<ComponentTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<ComponentTab> responseModel = new ResponseDataPageListForBootModel<ComponentTab>();
        ...
        ComponentTabExample example = new ComponentTabExample();
        ...
        PageInfo<ComponentTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_ComponentTab_QUERY_LIST")
    //@Cacheable(value = "ComponentTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/ComponentTab/getList", roleName = "组件_查询_列表", 
					roleFlag = "ROLE_ComponentTab_QUERY_LIST")
    public ResponseDataListModel<ComponentTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<ComponentTab> responseModel = new ResponseDataListModel<ComponentTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<ComponentTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_ComponentTab_ADD")
	@CachePut(value = "ComponentTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ComponentTab/add", roleName = "组件_添加", 
				roleFlag = "ROLE_ComponentTab_ADD")
    public ResponseDataModel<ComponentTab> add(@RequestBody ComponentTab model) {
        ResponseDataModel<ComponentTab> responseModel = new ResponseDataModel<ComponentTab>();
        try {
            ComponentTab record = new ComponentTab();
	...
	@Secured("ROLE_ComponentTab_EDIT")
	@CachePut(value = "ComponentTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ComponentTab/update", roleName = "组件_编辑", 
			roleFlag = "ROLE_ComponentTab_EDIT")
    public ResponseDataModel<ComponentTab> update(@RequestBody ComponentTab model) {
        ResponseDataModel<ComponentTab> responseModel = new ResponseDataModel<ComponentTab>();
        try {
            ComponentTab record = new ComponentTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_ComponentTab_DELETE")
	@CacheEvict(value = "ComponentTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ComponentTab/del", roleName = "组件_删除", 
				roleFlag = "ROLE_ComponentTab_DELETE")
