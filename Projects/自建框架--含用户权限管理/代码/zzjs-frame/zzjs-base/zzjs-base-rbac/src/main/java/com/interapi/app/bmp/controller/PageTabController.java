...
import com.interapi.app.bmp.model.PageTabExample.Criteria;
...
@RestController("PageTabController")
@RequestMapping("/api/PageTab")
@ResourceMapping(code = "PageTab", name = "菜单页面_模块", tableName = "page_tab", menuName = "基础信息管理",
roleName = "菜单页面_页面", roleFlag = "ROLE_PageTab_PAGE", homePage = "/pages/PageTab",
pageSort = "4", menuSort = "2")
public class PageTabController {

    public static Logger logger = LoggerFactory.getLogger(PageTabController.class);

    @Autowired
    private PageTabService<PageTab, PageTabExample> service;
	
	@Secured("ROLE_PageTab_QUERY_DETAIL")
	@Cacheable(value = "PageTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/PageTab/get", roleName = "菜单页面_查询_详情", 
					roleFlag = "ROLE_PageTab_QUERY_DETAIL")
    public ResponseDataModel<PageTab> get(Long id) {
        ResponseDataModel<PageTab> responseModel = new ResponseDataModel<>();
        try {
            PageTab entity=service.get(id);
	...
	@Secured("ROLE_PageTab_QUERY_PAGE")
	//@Cacheable(value = "PageTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/PageTab/getListByPage", roleName = "菜单页面_查询_分页", 
					roleFlag = "ROLE_PageTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<PageTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<PageTab> responseModel = new ResponseDataPageListForBootModel<PageTab>();
        ...
        PageTabExample example = new PageTabExample();
        ...
        PageInfo<PageTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_PageTab_QUERY_LIST")
    //@Cacheable(value = "PageTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/PageTab/getList", roleName = "菜单页面_查询_列表", 
					roleFlag = "ROLE_PageTab_QUERY_LIST")
    public ResponseDataListModel<PageTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<PageTab> responseModel = new ResponseDataListModel<PageTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<PageTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_PageTab_ADD")
	@CachePut(value = "PageTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/PageTab/add", roleName = "菜单页面_添加", 
				roleFlag = "ROLE_PageTab_ADD")
    public ResponseDataModel<PageTab> add(@RequestBody PageTab model) {
        ResponseDataModel<PageTab> responseModel = new ResponseDataModel<PageTab>();
        try {
            PageTab record = new PageTab();
	...
	@Secured("ROLE_PageTab_EDIT")
	@CachePut(value = "PageTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/PageTab/update", roleName = "菜单页面_编辑", 
			roleFlag = "ROLE_PageTab_EDIT")
    public ResponseDataModel<PageTab> update(@RequestBody PageTab model) {
        ResponseDataModel<PageTab> responseModel = new ResponseDataModel<PageTab>();
        try {
            PageTab record = new PageTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_PageTab_DELETE")
	@CacheEvict(value = "PageTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/PageTab/del", roleName = "菜单页面_删除", 
				roleFlag = "ROLE_PageTab_DELETE")
