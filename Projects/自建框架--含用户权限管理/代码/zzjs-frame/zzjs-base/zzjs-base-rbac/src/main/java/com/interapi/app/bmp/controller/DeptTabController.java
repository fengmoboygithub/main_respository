...
import com.interapi.app.bmp.model.DeptTabExample.Criteria;
...
@RestController("DeptTabController")
@RequestMapping("/api/DeptTab")
@ResourceMapping(code = "DeptTab", name = "部门_模块", tableName = "dept_tab", menuName = "组织结构管理",
roleName = "部门_页面", roleFlag = "ROLE_DeptTab_PAGE", homePage = "/pages/DeptTab",
pageSort = "2", menuSort = "3")
public class DeptTabController {

    public static Logger logger = LoggerFactory.getLogger(DeptTabController.class);

    @Autowired
    private DeptTabService<DeptTab, DeptTabExample> service;
	
	@Secured("ROLE_DeptTab_QUERY_DETAIL")
	@Cacheable(value = "DeptTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DeptTab/get", roleName = "部门_查询_详情", 
					roleFlag = "ROLE_DeptTab_QUERY_DETAIL")
    public ResponseDataModel<DeptTab> get(Long id) {
        ResponseDataModel<DeptTab> responseModel = new ResponseDataModel<>();
        try {
            DeptTab entity=service.get(id);
	...
	@Secured("ROLE_DeptTab_QUERY_PAGE")
	//@Cacheable(value = "DeptTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/DeptTab/getListByPage", roleName = "部门_查询_分页", 
					roleFlag = "ROLE_DeptTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<DeptTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<DeptTab> responseModel = new ResponseDataPageListForBootModel<DeptTab>();
        ...
        DeptTabExample example = new DeptTabExample();
        ...
        PageInfo<DeptTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_DeptTab_QUERY_LIST")
    //@Cacheable(value = "DeptTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/DeptTab/getList", roleName = "部门_查询_列表", 
					roleFlag = "ROLE_DeptTab_QUERY_LIST")
    public ResponseDataListModel<DeptTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<DeptTab> responseModel = new ResponseDataListModel<DeptTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<DeptTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_DeptTab_ADD")
	@CachePut(value = "DeptTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DeptTab/add", roleName = "部门_添加", 
				roleFlag = "ROLE_DeptTab_ADD")
    public ResponseDataModel<DeptTab> add(@RequestBody DeptTab model) {
        ResponseDataModel<DeptTab> responseModel = new ResponseDataModel<DeptTab>();
        try {
            DeptTab record = new DeptTab();
	...
	@Secured("ROLE_DeptTab_EDIT")
	@CachePut(value = "DeptTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DeptTab/update", roleName = "部门_编辑", 
			roleFlag = "ROLE_DeptTab_EDIT")
    public ResponseDataModel<DeptTab> update(@RequestBody DeptTab model) {
        ResponseDataModel<DeptTab> responseModel = new ResponseDataModel<DeptTab>();
        try {
            DeptTab record = new DeptTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_DeptTab_DELETE")
	@CacheEvict(value = "DeptTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/DeptTab/del", roleName = "部门_删除", 
				roleFlag = "ROLE_DeptTab_DELETE")
