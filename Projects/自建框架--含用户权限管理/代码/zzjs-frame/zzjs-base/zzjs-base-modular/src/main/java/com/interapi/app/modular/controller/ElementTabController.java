...
import com.interapi.app.modular.model.ElementTabExample.Criteria;
...
@RestController("ElementTabController")
@RequestMapping("/api/ElementTab")
@ResourceMapping(code = "ElementTab", name = "元素_模块", tableName = "element_tab", menuName = "模块管理",
roleName = "元素_页面", roleFlag = "ROLE_ElementTab_PAGE", homePage = "/pages/ElementTab",
pageSort = "7", menuSort = "1")
public class ElementTabController {

    public static Logger logger = LoggerFactory.getLogger(ElementTabController.class);

    @Autowired
    private ElementTabService<ElementTab, ElementTabExample> service;
	
	@Secured("ROLE_ElementTab_QUERY_DETAIL")
	@Cacheable(value = "ElementTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ElementTab/get", roleName = "元素_查询_详情", 
					roleFlag = "ROLE_ElementTab_QUERY_DETAIL")
    public ResponseDataModel<ElementTab> get(Long id) {
        ResponseDataModel<ElementTab> responseModel = new ResponseDataModel<>();
        try {
            ElementTab entity=service.get(id);
	...
	@Secured("ROLE_ElementTab_QUERY_PAGE")
	//@Cacheable(value = "ElementTab")
    @PostMapping(value = "/getListByPage", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/ElementTab/getListByPage", roleName = "元素_查询_分页", 
					roleFlag = "ROLE_ElementTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<ElementTab> getListByPage(
        ...
        ResponseDataPageListForBootModel<ElementTab> responseModel = new ResponseDataPageListForBootModel<ElementTab>();
        ...
        ElementTabExample example = new ElementTabExample();
        ...
        PageInfo<ElementTab> page = service.getListByPage(currentPage, itemsPerPage, example);
        responseModel.setSuccess(true);
    ...
    @Secured("ROLE_ElementTab_QUERY_LIST")
    //@Cacheable(value = "ElementTab")
    @PostMapping(value = "/getList", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/ElementTab/getList", roleName = "元素_查询_列表", 
					roleFlag = "ROLE_ElementTab_QUERY_LIST")
    public ResponseDataListModel<ElementTab> getList(@RequestParam(required = false) String param)  throws BaseException{
        logger.info("param:{}", param);
        ResponseDataListModel<ElementTab> responseModel = new ResponseDataListModel<ElementTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<ElementTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@Secured("ROLE_ElementTab_ADD")
	@CachePut(value = "ElementTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ElementTab/add", roleName = "元素_添加", 
				roleFlag = "ROLE_ElementTab_ADD")
    public ResponseDataModel<ElementTab> add(@RequestBody ElementTab model) {
        ResponseDataModel<ElementTab> responseModel = new ResponseDataModel<ElementTab>();
        try {
            ElementTab record = new ElementTab();
	...
	@Secured("ROLE_ElementTab_EDIT")
	@CachePut(value = "ElementTab", key = "#root.caches[0].name + ':' + #model.id")
    @PutMapping(value = "/update", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ElementTab/update", roleName = "元素_编辑", 
			roleFlag = "ROLE_ElementTab_EDIT")
    public ResponseDataModel<ElementTab> update(@RequestBody ElementTab model) {
        ResponseDataModel<ElementTab> responseModel = new ResponseDataModel<ElementTab>();
        try {
            ElementTab record = new ElementTab();
            BeanUtils.copyProperties(model, record);
	...
	@Secured("ROLE_ElementTab_DELETE")
	@CacheEvict(value = "ElementTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/ElementTab/del", roleName = "元素_删除", 
				roleFlag = "ROLE_ElementTab_DELETE")
