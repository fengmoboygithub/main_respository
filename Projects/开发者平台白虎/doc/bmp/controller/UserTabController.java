...
import com.interapi.app.bmp.model.UserTabExample.Criteria;
...
@RestController("UserTabController")
@RequestMapping("/api/UserTab")
@ResourceMapping(code = "UserTab", name = "用户_模块", tableName = "user_tab", menuName = "基础信息管理",
roleName = "用户_页面", roleFlag = "ROLE_UserTab_PAGE", homePage = "/pages/UserTab/list.html",
pageSort = "1", menuSort = "2")
public class UserTabController {

    public static Logger logger = LoggerFactory.getLogger(UserTabController.class);

    @Autowired
    private UserTabService service;
	
	@Cacheable(value = "UserTab")
    @PostMapping(value = "/get", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/UserTab/get", roleName = "用户_查询_详情", 
					roleFlag = "ROLE_UserTab_QUERY_DETAIL")
    public ResponseDataModel<UserTab> get(Long id) {
        ResponseDataModel<UserTab> responseModel = new ResponseDataModel<>();
        try {
            UserTab entity=service.getUserTab(id);
    ...
    @ResourceMapping(code = "/api/UserTab/getListByPage", roleName = "用户_查询_分页", 
					roleFlag = "ROLE_UserTab_QUERY_PAGE")
    public ResponseDataPageListForBootModel<UserTab> getListByPage(
    ...
    @ResponseBody
    @SuppressWarnings("unchecked")
    @ResourceMapping(code = "/api/UserTab/getList", roleName = "用户_查询_列表", 
					roleFlag = "ROLE_UserTab_QUERY_LIST")
    public ResponseDataListModel<UserTab> getList(@RequestParam(required = false) String param) {
        logger.info("param:{}", param);
        ResponseDataListModel<UserTab> responseModel = new ResponseDataListModel<UserTab>();
        HashMap<String, Object> paramMap = new HashMap<>();
        if (!StringUtils.isEmpty(param)) {
            try {
                paramMap = new ObjectMapper().readValue(param, HashMap.class);
        ...
        List<UserTab> list = service.getList(example);
        responseModel.setSuccess(true);
	...
	@CachePut(value = "UserTab", key = "#root.caches[0].name + ':' + #model.id")
    @PostMapping(value = "/add", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/UserTab/add", roleName = "用户_添加", 
				roleFlag = "ROLE_UserTab_ADD")
    public ResponseModel add(@RequestBody UserTab model) {
    ...
    @ResourceMapping(code = "/api/UserTab/update", roleName = "用户_编辑", 
			roleFlag = "ROLE_UserTab_EDIT")
    public ResponseModel update(@RequestBody UserTab model) {
        ResponseModel responseModel = new ResponseModel();
        try {
            UserTab record = new UserTab();
            BeanUtils.copyProperties(model, record);
	...
	@CacheEvict(value = "UserTab")
    @DeleteMapping(value = "/del", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResourceMapping(code = "/api/UserTab/del", roleName = "用户_删除", 
				roleFlag = "ROLE_UserTab_DELETE")
